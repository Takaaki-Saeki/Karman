...
!
     implicit none
!
     integer::i,j,icent,jcent,n,nstep,nbegin,itr,itrp
	 double precision::time,resp,cptop,cpbtm,cpfore,cpback,cp1,cp2,cd,cl,dp,res,ux,uy,vx,vy
!
  end module others
!
!-----------------
...
! caluculate CD and CL
        cd=0.0
		do j=j1,j2-1
		   cpfore=(2.*p(i1,j)+2.*p(i1,j+1))/2.     ! Cpを積分
		   cpback=(2.*p(i2,j)+2.*p(i2,j+1))/2.     ! Cp=2pである
		   cd=cd+(cpfore-cpback)*dy
        end do
        cl=0.0
		do i=i1,i2-1
		   cpbtm=(2.*p(i,j1)+2.*p(i+1,j1))/2.
		   cptop=(2.*p(i,j2)+2.*p(i+1,j2))/2.
...
! 結果をnlpステップおきにモニター
        if((n/nlp)*nlp==n) then
		   ...
		   write(6,601) nstep,resp,itrp,cd,cl,cp1,cp2
		   write(70,602) nstep,resp,cd,cl,cp1,cp2
		   do i=1,mx
		      do j=1,my
                 write(40,*) p(i,j)
...
! (1)compute RHS(右辺の計算)
     do i=2,mx-1
	    do j=2,my-1
		   if(i<i1.or.i>i2.or.j<j1.or.j>j2) then  ! 正方形内部は計算しない
